...
//	public static final char ON_PATH = 'x';  // Lies on the path found
	...
	public static final char WALL = '#';     // Represents a wall
	public static final char START = 'S';    // Start position in the maze (only one)
	public static final char TARGET = 'X';   // Target (goal) position in the maze.
	public static final char EXPLORED = '!'; // Has been explored.
	public static final char ON_PATH = '+';  // Lies on the path found
	public static final char ON_PATHUP = 'U';  // Lies on the path found up
	public static final char ON_PATHDOWN = 'D';  // Lies on the path found down
	public static final char ON_PATHRIGHT = 'R';  // Lies on the path found right
	public static final char ON_PATHLEFT = 'L';  // Lies on the path found left
	public  String[] directionArr = new String[100];
			...
			if (!seq.hasNext())
				...
				if (theMaze[row][col] == TARGET) {
				  ...
				  //tracePath(current.getPrevious());
					traceDirectionPath(current.getPrevious());
	...
	private void traceDirectionPath(SequenceItem last)
	{
		int i = 0;
		for (SequenceItem cur = last;
			 cur.getPrevious() != null;
			 cur = cur.getPrevious()) {
			//Down
			if (cur.getRow() > cur.getPrevious().getRow()) {
				theMaze[cur.getRow()][cur.getCol()] = ON_PATHDOWN;
				directionArr[i] = String.valueOf(ON_PATHDOWN);
			}
			//Up
			if (cur.getRow() < cur.getPrevious().getRow()) {
				theMaze[cur.getRow()][cur.getCol()] = ON_PATHUP;
				directionArr[i] = String.valueOf(ON_PATHUP);
			}
			//Right
			if (cur.getCol() > cur.getPrevious().getCol()) {
				theMaze[cur.getRow()][cur.getCol()] = ON_PATHRIGHT;
				directionArr[i] = String.valueOf(ON_PATHRIGHT);
			}
			//Left
			if (cur.getCol() < cur.getPrevious().getCol()) {
				theMaze[cur.getRow()][cur.getCol()] = ON_PATHLEFT;
				directionArr[i] = String.valueOf(ON_PATHLEFT);
			}
		i++;
		}
	...
	public  String directionOutputArr()
	{

		String destination = "";
		int arrLen = initLength() - 1;
		for(int i = arrLen; i >= 0; i--)
		{
			destination += directionArr[i] + ", ";
		}
		return destination;
	}
	...
	public static void main(String[] args) {
		...
		String a = maze.directionOutputArr();
		System.out.println("Directions: " + a);
		System.out.println("Direction steps: " + maze.initLength());
